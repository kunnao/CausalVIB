...
#
# class Trainer_BinaryT:
#
#     def __init__(self, network_type, dataset, targeted_regularization, batch_size, directory, save_model_dir,
#                  output_dir,
#                  folder,
#                  plot_result=True,
#                  epoch=100, validation_frequency=1, patience=5, min_delta=1e-5, verbose=1):
...
#                 if dataset is None:
#                     break
#                 train, test, ufid_output_file, ufid, y0_scaler, y1_scaler = \
#                     dataset[0], dataset[1], dataset[2], dataset[3], dataset[4], dataset[5]
#                 x_train, y0_train, y1_train, t0_train, t1_train = train[0], train[1], train[2], train[3], train[4]
#                 x_test, y0_test, y1_test, t0_test, t1_test = test[0], test[1], test[2], test[3], test[4]
#                 # self.__batch_size = math.ceil(train[1].shape[0] * 0.7 // 10)
#                 sgd = 1
...
#
#                 model.compile(optimizer=Adam(lr=1e-3), loss=loss, metrics=metrics)
#                 training_history = model.fit(x=np.concatenate((x_train, y0_train, y1_train, t0_train, t1_train), axis=1),
#                                              y=np.concatenate((y0_test, y1_test, t0_test, t1_test), axis=1),
#                                              batch_size=self.__batch_size, epochs=self.__epoch,
#                                              verbose=self.__verbose, callbacks=adam_callbacks,
#                                              validation_split=self.__val_split)
#                 # global adam_model
...
#                     model.compile(optimizer=SGD(lr=sgd_lr, momentum=0.9, nesterov=True), loss=loss,
#                                   metrics=metrics)
#                     history1 = model.fit(x=np.concatenate((x_train, y0_train, y1_train, t0_train, t1_train), axis=1),
#                                          y=np.concatenate((y0_test, y1_test, t0_test, t1_test), 1), callbacks=sgd_callbacks,
#                                          validation_split=self.__val_split, epochs=self.__epoch,
#                                          batch_size=self.__batch_size, verbose=self.__verbose)
#                     loss_hst = history1.history["loss"][-1]
#                     if sgd_lr < 1e-10:
...
#                 yt_hat_test = model.predict(np.concatenate((x_test, y0_test, y1_test, t0_test, t1_test), axis=1))
#                 yt_hat_train = model.predict(np.concatenate((x_train, y0_train, y1_train, t0_train, t1_train), axis=1))
#                 self.__test_outputs += [
#                     _split_output_binT(yt_hat_test, test[2], test[1], test[3], test[0], test[4])]
#                 self.__train_outputs += [
#                     _split_output_binT(yt_hat_train, train[2], train[1], test[3], train[0], train[3])]
#                 K.clear_session()
#             if self.__dataset == 'acic':
#                 if self.__targeted_regularization:
#                     train_output_dir = os.path.join(ufid_output_file,
...
#
#
# def _split_output_binT(yt_hat, y0, y1, t0, t1, y0_scaler, y1_scaler, x, index):
#     y0 = y0_scaler.inverse_transform(yt_hat[:, 0].copy())
#     y1 = y1_scaler.inverse_transform(yt_hat[:, 1].copy())
#     g = yt_hat[:, 2].copy()
#
#     if yt_hat.shape[1] >= 4:
#         eps = yt_hat[:, 3][0]
